...
public partial class DeviceCommunicationPageViewModel : ObservableObject, IDisposable {
    private static readonly ILogger Logger = Log.ForContext<DeviceCommunicationPageViewModel>();
    private readonly IDeviceDiscoveryService _deviceDiscoveryService;
    private readonly IMessageAppService _messageAppService;
    private readonly IChatAttachmentStore _attachmentStore;
    private readonly IChatClipboardService? _clipboardService;
    ...
    private readonly IChatNotificationSink _notificationSink;
    ...
    public DeviceCommunicationPageViewModel(
        ...
        IChatNotificationSink notificationSink,
        IChatClipboardService? clipboardService = null,
        ...
        _postToUi = postToUi ?? PostToAvaloniaUiThread;
    ...
    private async Task LoadFileIconAsync(FileChatMessageItem item, string path)
    {
        try
        {
            var iconPng = await Task.Run(() => _attachmentStore.GetFileIconPng(path));
            var icon = CreateBitmap(iconPng);
            if (icon is not null)
            {
                ExecuteOnUiThread(() => item.FileIcon = icon);
            }
        }
        catch (Exception ex)
        {
            Logger.Debug(ex, "Load file icon failed. Path={Path}", path);
        }
    }
    ...
    private async Task PasteSendAsync() {
        ...
        var clipboard = await _clipboardService.ReadAsync();

        var text = clipboard.Text?.Trim();
        ...
        if (clipboard.ImagePng is { Length: > 0 } imageBytes) {
            try {
                    var imageBubble =
    ...
    private void SendFilesAsync(DeviceConversationItem conversation, IReadOnlyCollection<string> filePaths,
        ...
        foreach (var filePath in filePaths) {
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    var iconPng = _attachmentStore.GetFileIconPng(capturedPath);
                    var icon = CreateBitmap(iconPng);
                    if (icon is not null)
                    {
                        ExecuteOnUiThread(() => capturedBubble.FileIcon = icon);
                    }
    ...
    private async Task AcceptIncomingOfferAsync(FileChatMessageItem? messageItem) {
        ...
        try {
            var saveTarget = await _attachmentStore.PickSaveTargetAsync(offer.FileName);
    ...
    private static void PostToAvaloniaUiThread(Action action)
    {
    ...
    private bool TryGetConversation(
        string conversationId,
        [NotNullWhen(true)] out DeviceConversationItem? conversation) {
    ...
    private void OnDiscoveredDevicesCollectionChanged(object? sender, NotifyCollectionChangedEventArgs e) {
        ...
        ExecuteOnUiThread(SyncConversationsFromDiscovery);
    }
    ...
    private void OnChatMessageReceived(TextChatMessage message, DateTimeOffset timestampUtc) {
        ...
        ExecuteOnUiThread(() => {
            ...
            if (!IsForegroundCurrentConversation(conversation)) {
                ...
                ShowIncomingNotification(conversation, text);
            }
            else {
                conversation.UnreadCount = 0;
            }
    ...
    private void OnFileOfferReceived(FileTransferUpdatedEvent message) {
        ...
        ExecuteOnUiThread(() => {
            ...
            if (fileBubble.FileIcon is null)
            {
                ...
                if (ext is not null)
                {
                    _ = LoadFileIconAsync(fileBubble, $"file{ext}");
                }
            }
    ...
    {
        if (_notificationSink.IncomingMessagesHandledExternally)
        {
            return;
        }

        _ = _notificationSink.ShowAsync(conversation.DisplayName, preview);
    }
