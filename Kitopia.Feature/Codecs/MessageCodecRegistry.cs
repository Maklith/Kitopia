...
namespace Kitopia.Feature.DeviceCommunication.Codecs;
...
public sealed class MessageCodecRegistry
{
    ...
    public MessageCodecRegistry(IDeviceIdentityProvider? deviceIdentityProvider = null)
    {
    ...
    private bool TryEncodeTextChat(TextChatMessage message, out DataEnvelope envelope)
    {
    ...
    private bool TryEncodeFileChat(FileChatMessage message, out DataEnvelope envelope)
    {
    ...
    private bool TryEncodeImageChat(ImageChatMessage message, out DataEnvelope envelope)
    {
        if (!message.IsDirect)
        {
            return Fail(out envelope);
    ...
    private bool TryEncodeFileOffer(FileOfferChatMessage message, out DataEnvelope envelope)
    {
    ...
    private bool TryEncodeTransferControl(
        AppMessage message,
        string command,
        Guid transferId,
        string? reason,
    ...
    private static bool TryDecodeFileOffer(DataEnvelope envelope, out AppMessage message)
    {
        ...
        if (!string.IsNullOrWhiteSpace(encodedIcon))
        {
            try
            {
                iconPng = Convert.FromBase64String(encodedIcon);
            }
            catch (FormatException)
            {
                // Invalid optional icon metadata must not reject the file offer itself.
            }
        }
    ...
    private string ResolveSenderId()
    {
        try
        {
            return _deviceIdentityProvider?.GetLocalPublicKey() ?? string.Empty;
        }
        catch
        {
            return string.Empty;
