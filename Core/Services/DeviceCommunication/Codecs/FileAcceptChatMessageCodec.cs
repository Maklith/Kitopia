...
public sealed class FileAcceptChatMessageCodec : IMessageCodec
{
    public string Route => "chat";
    public string Command => "file.accept";
    public Type MessageType => typeof(FileAcceptChatMessage);

    public bool TryEncode(AppMessage message, out DataEnvelope envelope)
    {
        if (message is not FileAcceptChatMessage accept)
        {
    ...
    public bool TryDecode(DataEnvelope envelope, out AppMessage message)
    {
        ...
        message = new FileAcceptChatMessage(conversationId!, envelope.ChannelId);
        return true;
    }
