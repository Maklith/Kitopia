...
public sealed class ClipboardSyncService : IClipboardSyncService {
    ...
    public DeviceModel? TargetDevice => _targetDevice;
    ...
    public async Task<bool> EnableAsync(DeviceModel target, TimeSpan timeout, string duplicateError) {
        ...
        UpdateState(
            ...
            status: $"等待 {resolvedTarget} 同意同步请求...",
        ...
        if (!IsEnabled || TargetDevice is not { } current || !current.Equals(resolvedTarget)) {
            Activate(resolvedTarget, $"已与 {resolvedTarget} 建立双向剪贴板同步");
        }
    ...
    public void ApplyIncomingClipboardText(DeviceModel sender, string text) {
        ...
        if (!enabled || target is null || !target.Equals(sender)) {
            return;
        }
    ...
    public async Task HandleIncomingRequestAsync(
        ...
        Activate(sender, $"已同意 {sender} 的请求，双向同步已开启");
        Authorized?.Invoke(this, new DeviceClipboardSyncAuthorizedEventArgs(sender, true));
    }
    ...
    public Task HandleIncomingResponseAsync(PacketMetadata packet, DeviceModel sender) {
        ...
        Activate(sender, $"对方已同意，已与 {sender} 开启双向同步");
        Authorized?.Invoke(this, new DeviceClipboardSyncAuthorizedEventArgs(sender, false));
        return Task.CompletedTask;
    }
                ...
                if (enabled && target is not null) {
                    ...
                    if (!isApplyingRemote) {
                        ...
                        if (!string.IsNullOrEmpty(currentText)) {
                            ...
                            bool suppressedEcho;
                            lock (_syncLock) {
                                shouldSend = !string.Equals(currentText, _lastSyncedClipboardText,
                                    StringComparison.Ordinal);
                                suppressedEcho = shouldSend &&
                                                 string.Equals(currentText, _lastInboundClipboardText,
                                                     StringComparison.Ordinal) &&
                                                 DateTime.UtcNow - _lastInboundClipboardUtc <=
                                                 ClipboardEchoSuppressionWindow;
                                if (suppressedEcho) {
                                    shouldSend = false;
                                }
