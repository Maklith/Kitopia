...
public sealed class MessageAppService : IMessageAppService {
    ...
    private Task NotifyToastIfNeededAsync(DeviceMessageEvent messageEvent) {
        ...
        switch (messageEvent) {
            ...
            case FileTransferUpdatedEvent { Status: FileTransferStatus.WaitingForAccept } fileOffer:
                ShowIncomingFileOfferToast(conversationId, displayName, fileOffer.TransferId, fileOffer.FileName,
                    fileOffer.TotalBytes);
                break;
            case FileTransferUpdatedEvent { Direction: FileTransferDirection.Download } downloadEvent
                when downloadEvent.Status is FileTransferStatus.InProgress or FileTransferStatus.Completed or FileTransferStatus.Rejected or FileTransferStatus.Failed:
    ...
    private void ShowIncomingFileOfferToast(string conversationId, string displayName, Guid transferId, string? fileName,
        long? totalBytes) {
        var resolvedFileName = string.IsNullOrWhiteSpace(fileName) ? transferId.ToString("D") : fileName;
        var text = $"文件: {resolvedFileName} ({FormatFileSize(totalBytes ?? 0)})";
        ...
        _toastService.Show(new ToastRequest {
            ...
            Actions = new[] {
                new ToastAction {
                    ...
                    IsPrimary = true,
                    CloseOnClick = true,
                    Callback = () => _ = Task.Run(() => AcceptIncomingOfferFromToastAsync(
                        conversationId,
                        displayName,
                        transferId,
                        resolvedFileName))
                ...
                new ToastAction {
                    Text = "拒绝",
                    CloseOnClick = true,
                    Callback = () => _ = Task.Run(() => RejectIncomingOfferFromToastAsync(conversationId, transferId))
                },
                new ToastAction {
                    Text = "打开聊天",
                    CloseOnClick = true,
    ...
    private async Task AcceptIncomingOfferFromToastAsync(string conversationId, string displayName, Guid transferId,
        string fileName) {
        try {
            var savePath = await PickSavePathAsync(fileName);
            if (string.IsNullOrWhiteSpace(savePath) || !Path.IsPathRooted(savePath)) {
                return;
            }

            await AcceptFileAsync(conversationId, transferId, savePath);
            ShowIncomingProgressToast(transferId, displayName, fileName);
        }
        catch (Exception ex) {
            Logger.Warning(ex, "Accept incoming offer from toast failed. ConversationId={ConversationId} TransferId={TransferId}",
                conversationId, transferId);
            _toastService.Show("设备聊天", $"同意接收失败: {ex.Message}", NotificationType.Error);
        }
    }

    private async Task RejectIncomingOfferFromToastAsync(string conversationId, Guid transferId) {
        try {
            await RejectFileAsync(conversationId, transferId, "rejected_by_user");
        }
        catch (Exception ex) {
            Logger.Warning(ex, "Reject incoming offer from toast failed. ConversationId={ConversationId} TransferId={TransferId}",
                conversationId, transferId);
            _toastService.Show("设备聊天", $"拒绝接收失败: {ex.Message}", NotificationType.Error);
        }
    }
    ...
    private void ShowIncomingProgressToast(Guid transferId, string displayName, string fileName) {
        lock (_stateSync) {
            if (_incomingTransferToasts.TryGetValue(transferId, out var existingHandle)) {
                existingHandle.Update(progress: 0, text: $"接收中: {fileName}", header: $"设备聊天:{displayName}",
            ...
            var handle = _toastService.ShowProgress(
                ...
                initialProgress: 0,
                isIndeterminate: false);
            _incomingTransferToasts[transferId] = handle;
        }
    }
    ...
    private void UpdateIncomingTransferToast(string displayName, FileTransferUpdatedEvent transferEvent) {
        ...
        switch (transferEvent.Status) {
            case FileTransferStatus.InProgress: {
                ...
                var progress = CalculateProgressPercent(transferEvent.BytesTransferred, transferEvent.TotalBytes);
                handle.Update(progress: progress, text: $"接收中: {fileName}", header: $"设备聊天:{displayName}",
                    isIndeterminate: progress < 0);
                break;
            }
            case FileTransferStatus.Completed: {
                if (handle is not null) {
                    handle.Complete($"接收完成: {fileName}", $"设备聊天:{displayName}", TimeSpan.FromSeconds(4));
                }
            ...
            case FileTransferStatus.Failed: {
                if (handle is not null) {
                    handle.Fail($"接收失败: {fileName}", $"设备聊天:{displayName}", TimeSpan.FromSeconds(5));
                }
                else {
                    ShowDeviceChatToast(transferEvent.ConversationId, displayName, $"接收失败: {fileName}");
                }
    ...
    private static double CalculateProgressPercent(long? bytesTransferred, long? totalBytes) {
        ...
        var ratio = Math.Clamp((double)bytesTransferred.Value / totalBytes.Value, 0d, 1d);
        return ratio * 100d;
    }
