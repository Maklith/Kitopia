...
public class CustomScenarioSearcherWrapper{
    ...
    public CustomScenario CustomScenario { get; set; }
}

public class CustomScenarioIdentifier : IInputDataIdentifier
{
    List<CustomScenarioSearcherWrapper> _cache = new();
    private PinyinSearcher<CustomScenarioSearcherWrapper> _pinyinSearcher;
    public CustomScenarioIdentifier()
    {
        _pinyinSearcher = new PinyinSearcher<CustomScenarioSearcherWrapper>(_cache, nameof(CustomScenarioSearcherWrapper.PinyinItem));
        
    }
    public void UpdateCache(object? sender, EventArgs eventArgs)
    {
        if (sender is CustomScenario scenario)
        {
            var customScenarioSearcherWrapper = _cache.Find( e=>e.CustomScenario==scenario);
            if (customScenarioSearcherWrapper != null)
            {
                _cache.Remove(customScenarioSearcherWrapper);
            }
        }
    ...
    public IEnumerable<PluginCore.SearchWindow.InputData.InputData> IdentifyInputData(IInputDataAnalyzeTimeFlags analyzeTimeFlags, string? s)
    {
        ...
        {
            if (_cache.Any(e=>e.CustomScenario==scenario))
            {
                continue;
            }
