...
public static class CaptureTool
{
    ...
    {
        if (screenCaptureInfo is { RequestRect: not null, ScreenInfo: not null }) {
            int startX = Math.Clamp(screenCaptureInfo.RequestRect.Value.X, 0, screenCaptureInfo.ScreenInfo.Value.Width - 1);
            int startY = Math.Clamp(screenCaptureInfo.RequestRect.Value.Y, 0, screenCaptureInfo.ScreenInfo.Value.Height - 1);
            int endX = Math.Clamp(screenCaptureInfo.RequestRect.Value.X + screenCaptureInfo.RequestRect.Value.Width, 0, screenCaptureInfo.ScreenInfo.Value.Width);
            int endY = Math.Clamp(screenCaptureInfo.RequestRect.Value.Y + screenCaptureInfo.RequestRect.Value.Height, 0, screenCaptureInfo.ScreenInfo.Value.Height);
